...
resp = requests.get("https://paas1.com:8443/api/v1/pods", verify=False, headers=headers)
json.dump(resp.json(), sys.stdout, indent=2)

if len(sys.argv) > 2:
    print('-'*30)
    requests.delete("https://122.115.45.68:8443/api/v1/namespaces/" + sys.argv[1] + "/pods/" + sys.argv[2], verify=False, headers=headers)
    json.dump(resp.json(), sys.stdout, indent=2)
