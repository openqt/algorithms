...
class Solution(object):
    def subdomainVisits(self, cpdomains):
        ...
        for domain in cpdomains:
            ...
            n, pos = int(n), 0
            while pos >= 0:
                data[s[pos:]] = data.setdefault(s[pos:], 0) + n
                pos = s.find('.', pos+1)
                if pos > 0: pos += 1
        return ["%s %s" % (j, i) for i, j in data.items()]


class T(unittest.TestCase):
    def test(self):
        s = Solution()
        self.assertEqual(sorted(s.subdomainVisits(
            ["9001 discuss.leetcode.com"])),
            sorted(["9001 discuss.leetcode.com", "9001 leetcode.com", "9001 com"]))
        self.assertEqual(sorted(s.subdomainVisits(
            ["900 google.mail.com", "50 yahoo.com", "1 intel.mail.com", "5 wiki.org"])),
            sorted(["901 mail.com", "50 yahoo.com", "900 google.mail.com", "5 wiki.org", "5 org",
