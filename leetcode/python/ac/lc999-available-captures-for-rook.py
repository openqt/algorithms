...
class Solution(object):
    def numRookCaptures(self, board):
        ...
        """
        n = 0
        x, y = self._findRock(board)
        n += self._p(board, range(x + 1, 8), [y])
        n += self._p(board, range(x-1, -1, -1), [y])
        n += self._p(board, [x], range(y + 1, 8))
        n += self._p(board, [x], range(y-1, -1, -1))
        return n

    def _p(self, board, x, y):
        n = 0
        for i in x:
            for j in y:
                if board[i][j] == ".":
